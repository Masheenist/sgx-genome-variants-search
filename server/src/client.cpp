...
void app_rhht(MsgIO *msgio, config_t &config, uint32_t nf, uint32_t nf_case,
              uint32_t csz, int k, int capacity, char *ofn, bool debug_flag) {
...
#else
      enclave_decrypt_process_rhht(ra_ctx, ciphertext, ciphertext_len);
#endif

      num_elems_rcvd = num_elems_rcvd + to_read_elems;
      num_elems_rem = num_elems_rem - to_read_elems;

      // We've processed the secret data, now either clean it up or use data
      // sealing for a second pass later
      delete[] ciphertext;
    }
  }
...
#else
  enclave_init_id_buf(k);
  enclave_init_res_buf(k);
  rhht_init_chi_sq(case_count, control_count, k);
#endif
...
#else
  enclave_get_res_pairs(chi_sq_pairs, k);
#endif
...
#else
  enclave_free_id_buf();
  enclave_free_res_buf();
#endif
}

void app_oa(MsgIO *msgio, config_t &config, uint32_t nf, uint32_t nf_case,
            uint32_t csz, int k, int capacity, char *ofn, bool debug_flag) {
  // Get the Enclave ID from the configuration
  auto &eid = config.eid;
...
#else
      enclave_decrypt_process_oa(ra_ctx, ciphertext, ciphertext_len);
#endif
      num_elems_rcvd = num_elems_rcvd + to_read_elems;
      num_elems_rem = num_elems_rem - to_read_elems;
...
// Make an ECALL to perform the chi-squared test
  ...
  enclave_init_id_buf(k);
  enclave_init_res_buf(k);
  oa_init_chi_sq(case_count, control_count, k);
#endif
...
void app_cmtf(MsgIO *msgio, config_t &config, uint32_t nf, uint32_t nf_case,
              uint32_t csz, int k, int nbuckets, char *ofn, bool debug_flag) {
  // Get the Enclave ID from the configuration
  auto &eid = config.eid;
...
#else
      enclave_decrypt_process_cmtf(ra_ctx, ciphertext, ciphertext_len);
#endif
      num_elems_rcvd = num_elems_rcvd + to_read_elems;
      num_elems_rem = num_elems_rem - to_read_elems;
...
// Make an ECALL to perform the chi-squared test
  ...
  enclave_init_id_buf(k);
  enclave_init_res_buf(k);
  cmtf_init_chi_sq(case_count, control_count, k);
#endif
...
void app_csk(MsgIO *msgio, config_t &config, uint32_t nf, uint32_t nf_case,
             uint32_t ncase, uint32_t ncontrol, uint32_t csz, int l, char *ofn,
             int k, int w, int d, int nt, bool cache_aware, bool debug_flag) {
  // Get the Enclave ID from the configuration
  auto &eid = config.eid;
  global_eid = eid;
...
#else
        enclave_decrypt_update_csk(ra_ctx, ciphertext, ciphertext_len);
#endif
        num_elems_rcvd = num_elems_rcvd + to_read_elems;
        num_elems_rem = num_elems_rem - to_read_elems;
...
#else
        enclave_decrypt_store_csk(ra_ctx, ciphertext, ciphertext_len);
#endif
...
#else
    enclave_decrypt_query_csk(ra_ctx, ciphertext, ciphertext_len);
#endif
    num_elems_rcvd = num_elems_rcvd + to_read_elems;
    num_elems_rem = num_elems_rem - to_read_elems;
  ...
  if (k <= 0) {
...
#else
    enclave_get_mh_ids(top_ids, l);
    enclave_get_mh_vals(acdiff_vals, l);
#endif
    ...
    for (i = 0; i < num_files; i++) {
      ...
      while (num_elems_rcvd != num_elems) {
        ...
        // Make an ECALL to decrypt the data and process it inside the Enclave
#ifndef NON_SGX
        enclave_decrypt_process_sketch_rhht(eid, ra_ctx, ciphertext,
                                            ciphertext_len);
#else
        enclave_decrypt_process_sketch_rhht(ra_ctx, ciphertext, ciphertext_len);
#endif
...
void app_cms(MsgIO *msgio, config_t &config, uint32_t nf, uint32_t nf_case,
             uint32_t ncase, uint32_t ncontrol, uint32_t csz, int l, char *ofn,
             int k, int w, int d, int nt, bool cache_aware, bool debug_flag) {
  // Get the Enclave ID from the configuration
  auto &eid = config.eid;
...
#else
        enclave_decrypt_update_cms(ra_ctx, ciphertext, ciphertext_len);
#endif
...
#else
        enclave_decrypt_store_cms(ra_ctx, ciphertext, ciphertext_len);
#endif
...
#else
    enclave_decrypt_query_cms(ra_ctx, ciphertext, ciphertext_len);
#endif
    num_elems_rcvd = num_elems_rcvd + to_read_elems;
    num_elems_rem = num_elems_rem - to_read_elems;
...
void app_svd_mcsk(MsgIO *msgio, config_t &config, uint32_t nf, uint32_t nf_case,
                  uint32_t csz, int l, char *ofn, int k, int w, int d,
                  int num_pc, float eps, bool sketching_flag, bool debug_flag) {
  // Get the Enclave ID from the configuration
  auto &eid = config.eid;
...
#else
  enclave_init_mcsk(nf, num_pc, eps);
#endif
...
#else
      enclave_decrypt_update_mcsk(ra_ctx, ciphertext, ciphertext_len);
#endif
      num_elems_rcvd = num_elems_rcvd + to_read_elems;
      num_elems_rem = num_elems_rem - to_read_elems;
...
#else
        enclave_decrypt_update_csk_f(ra_ctx, ciphertext, ciphertext_len);
#endif
        num_elems_rcvd = num_elems_rcvd + to_read_elems;
        num_elems_rem = num_elems_rem - to_read_elems;
...
#else
      enclave_decrypt_query_csk_f(ra_ctx, ciphertext, ciphertext_len);
#endif
      num_elems_rcvd = num_elems_rcvd + to_read_elems;
      num_elems_rem = num_elems_rem - to_read_elems;
...
#else
      enclave_decrypt_init_rhht_pcc(ra_ctx, ciphertext, ciphertext_len);
#endif
      num_elems_rcvd = num_elems_rcvd + to_read_elems;
      num_elems_rem = num_elems_rem - to_read_elems;
...
#else
      enclave_decrypt_process_rhht_pcc(ra_ctx, ciphertext, ciphertext_len);
#endif
      num_elems_rcvd = num_elems_rcvd + to_read_elems;
      num_elems_rem = num_elems_rem - to_read_elems;
...
// Make an ECALL to perform the chi-squared test
  ...
  enclave_init_id_buf(k);
  enclave_init_res_buf(k);
  rhht_init_cat_chi_sq(num_files, k);
#endif
      ...
      // Second token: variable value
        ...
        if (strcmp(var_name, "PORT_NUMBER") == 0) {
          ...
          if (strcmp((*params)->app_mode, "basic") != 0) {
            fprintf(stderr, "The parameter HASH_OPTION is applicable only in "
                            "mode basic.\n");
            exit(1);
          }
          ...
          if (strcmp((*params)->app_mode, "sketch") != 0) {
            fprintf(stderr, "The parameter SKETCH_MODE is applicable only in "
                            "mode sketch.\n");
            exit(1);
          }
          ...
          if ((strcmp((*params)->app_mode, "sketch") != 0) &&
            ...
            fprintf(
                stderr,
                "The parameter NUM_CASES is applicable only in mode sketch.\n");
            exit(1);
          }
          ...
          if ((strcmp((*params)->app_mode, "sketch") != 0) &&
              (strcmp((*params)->app_mode, "preprocess") != 0) &&
              (strcmp((*params)->app_mode, "full") != 0)) {
            fprintf(stderr, "The parameter NUM_CONTROLS is applicable only in "
                            "mode sketch.\n");
            exit(1);
          }
          ...
          if ((strcmp((*params)->app_mode, "sketch") != 0) &&
              (strcmp((*params)->app_mode, "preprocess") != 0) &&
              (strcmp((*params)->app_mode, "full") != 0)) {
            fprintf(stderr, "The parameter SKETCH_WIDTH is applicable only in "
                            "mode sketch.\n");
            exit(1);
          }
          ...
          if ((strcmp((*params)->app_mode, "sketch") != 0) &&
              (strcmp((*params)->app_mode, "preprocess") != 0) &&
              (strcmp((*params)->app_mode, "full") != 0)) {
            fprintf(stderr, "The parameter SKETCH_DEPTH is applicable only in "
                            "mode sketch.\n");
            exit(1);
          }
          ...
          if ((strcmp((*params)->app_mode, "preprocess") != 0) &&
              (strcmp((*params)->app_mode, "full") != 0)) {
            fprintf(stderr, "The parameter NUM_PC is applicable only in mode "
                            "pca_sketch.\n");
            exit(1);
          }
          ...
          if ((strcmp((*params)->app_mode, "sketch") != 0) &&
              (strcmp((*params)->app_mode, "full") != 0)) {
            fprintf(stderr, "The parameter NUM_TOP_CAND is applicable only in "
                            "mode sketch.\n");
            exit(1);
          }
          ...
          if (strcmp((*params)->app_mode, "sketch") != 0) {
            fprintf(stderr, "The parameter SKETCH_ROW_UPDATE is applicable "
                            "only in mode sketch.\n");
            exit(1);
          }
          ...
          if (strcmp((*params)->app_mode, "sketch") != 0) {
            fprintf(stderr, "The parameter SKETCH_CAND_ONLY is applicable only "
                            "in mode sketch.\n");
            exit(1);
          }
          ...
          if (strcmp((*params)->app_mode, "sketch") != 0) {
            fprintf(stderr, "The parameter NUM_THREADS is applicable only in "
                            "mode sketch.\n");
            exit(1);
          }
    ...
    if (strcmp(params->app_mode, "basic") == 0) {
      ...
      case 0:
        app_oa(msgio, config, params->num_files, params->num_files_case,
               params->chunk_size, params->k, params->init_capacity,
               params->output_file, false);
        break;
      case 2:
        app_cmtf(msgio, config, params->num_files, params->num_files_case,
                 params->chunk_size, params->k, params->num_buckets,
                 params->output_file, false);
        break;
      ...
      case 0:
        if (params->sketch_cand_only == 1) {
          app_cms(msgio, config, params->num_files, params->num_files_case,
                  ...
                  params->sketch_depth, params->num_threads, params->cache,
                  params->debug);
        } else {
          app_cms(msgio, config, params->num_files, params->num_files_case,
                  params->num_cases, params->num_controls, params->chunk_size,
                  params->l, params->output_file, params->k,
                  params->sketch_width, params->sketch_depth,
                  params->num_threads, params->cache, params->debug);
        }
        break;
      default:
        if (params->sketch_cand_only == 1) {
      ...
      if (params->num_pc / epsilon > 16000) {
        fprintf(stderr,
                "Temporarily not supporting this range of parameters.\n");
        exit(1);
      }
