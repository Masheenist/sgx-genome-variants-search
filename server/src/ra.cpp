  ...
  /*
      ...
      if (sgx_support & SGX_SUPPORT_NO) {
          ...
          if (sgx_support & SGX_SUPPORT_ENABLE_REQUIRED) {
              fprintf(stderr, "Intel SGX is supported on this system but
  disabled in the BIOS\n"); return 1;
          }
          else if (sgx_support & SGX_SUPPORT_REBOOT_REQUIRED) {
              fprintf(stderr, "Intel SGX will be enabled after the next
  reboot\n"); return 1;
          }
          else if (!(sgx_support & SGX_SUPPORT_ENABLED)) {
              fprintf(stderr, "Intel SGX is supported on this sytem but not
  available for use\n"); fprintf(stderr, "The system may lock BIOS support, or
  the Platform Software is not available\n"); return 1;
          }
      }
  #endif
  */
...
/* Launch the enclave */
  ...
  if (config.server == NULL) {
    *_msgio = new MsgIO();
    ...
    try {
      *_msgio = new MsgIO(config.server,
...
int do_attestation(sgx_enclave_id_t eid, config_t *config, MsgIO **_msgio) {
  ...
  if (OPT_ISSET(flags, OPT_PUBKEY)) {
    ...
    status = enclave_ra_init(eid, &sgxrv, config->pubkey, b_pse, &ra_ctx,
                             &pse_status);
#else
    enclave_ra_init(config->pubkey, b_pse, &ra_ctx, &pse_status);
...
#else
    enclave_ra_init_def(b_pse, &ra_ctx, &pse_status);
  ...
  if (retPibCmp == 0) {
    ...
    if (debug)
      eprintf("+++ sgx_report_attestation_status ret = 0x%4x\n", ret);
...
int do_quote(sgx_enclave_id_t eid, config_t *config) {
...
#else
    pse_manifest_sz = get_pse_manifest_size();
#endif
...
#else
    get_pse_manifest(pse_manifest, pse_manifest_sz);
#endif
  }
  ...
  if (CryptBinaryToString((BYTE *)quote, sz,
                          CRYPT_STRING_BASE64 | CRYPT_STRING_NOCRLF, NULL,
                          &sz_b64quote) == FALSE) {
    fprintf(stderr,
            "CryptBinaryToString: could not get Base64 encoded quote length\n");
    return 1;
  }

  b64quote = (LPTSTR)(malloc(sz_b64quote));
  if (CryptBinaryToString((BYTE *)quote, sz,
                          CRYPT_STRING_BASE64 | CRYPT_STRING_NOCRLF, b64quote,
                          &sz_b64quote) == FALSE) {
    fprintf(stderr,
            "CryptBinaryToString: could not get Base64 encoded quote length\n");
    return 1;
  ...
  if (OPT_ISSET(flags, OPT_PSE)) {
    if (CryptBinaryToString((BYTE *)pse_manifest, (uint32_t)(pse_manifest_sz),
                            CRYPT_STRING_BASE64 | CRYPT_STRING_NOCRLF, NULL,
                            &sz_b64manifest) == FALSE) {
      fprintf(stderr, "CryptBinaryToString: could not get Base64 encoded "
                      "manifest length\n");
      return 1;
    }
    ...
    if (CryptBinaryToString((BYTE *)pse_manifest, (uint32_t)(pse_manifest_sz),
                            ...
                            b64manifest, &sz_b64manifest) == FALSE) {
      fprintf(stderr, "CryptBinaryToString: could not get Base64 encoded "
                      "manifest length\n");
      return 1;
    }
